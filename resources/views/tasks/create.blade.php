...
<div class="max-w-xl mx-auto bg-white p-8 rounded shadow mt-8">
    ...
    <form action="{{ route('tasks.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="number" class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500" id="priority" name="priority" min="0" value="0">
        </div>
        <div>
            <label for="due_date" class="block text-sm font-medium text-gray-700">Due Date</label>
            <input type="date" class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500" id="due_date" name="due_date">
        </div>
        <div>
